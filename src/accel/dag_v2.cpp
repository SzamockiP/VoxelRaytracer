...
struct TempNode
{
    ...
    // Wypluwa węzeł do bufora surowego poziomu, redukując wskaźniki (Intra-Node)
    // Zwraca indeks (offset) wewnątrz level_buffer, pod którym zaczął się ten węzeł
    vrt::u32 emit(std::vector<vrt::u32>& level_buffer)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            vrt::u32 offset = 0;
            bool found = false;
            // Szukamy, czy ten wskaźnik już mamy w tym węźle (wewnętrzna deduplikacja)
            ...
            // Składamy kawałek: [1 bit Valid | 3 bity Offset]
...
#pragma pack(pop)
    ...
    while (file.read(reinterpret_cast<char*>(&voxel_data), sizeof(VoxelData)))
    {
        ...
        else
        {
            ...
            {
                // NAPRAWIONE: Zrzucamy poziom wyżej! (+ 1)
                node_idx = temp_nodes[current_level].emit(level_buffers[current_level + 1]);
                temp_nodes[current_level].clear();
                ++current_level;

                if (current_level >= depth - 1) break; // Zabezpieczenie przed błędem indeksu
    ...
    // Przepychanie węzłów aż do korzenia
    ...
    // =========================================================================
    // FAZA 2: KOMPRESJA BOTTOM-UP (Sortowanie pośrednie & Deduplikacja DAG)
    // =========================================================================

    std::unordered_map<u32, u32> remap_table; // Stary indeks -> Nowy absolutny indeks w nodes_
    ...
    for (int d = 0; d < depth; ++d)
    {
        ...
        // 4. Deduplikacja i wrzucanie do finalnego `nodes_`
        ...
        for (size_t s = 1; s < node_starts.size(); ++s)
        {
            ...
            // Sprawdzamy czy obecny i poprzedni posortowany element są w 100% identyczne
            bool is_same = true;
            ...
            if (is_same)
            {
                // Duplikat - odsyłamy stary wskaźnik do już istniejącego unikatu
                new_remap[curr_old] = last_unique_compact_start;
            }
            else
            {
                // Unikat - wypychamy fizycznie do nodes_
        ...
        remap_table = std::move(new_remap); // Zastępujemy remapę starym poziomem
    }
    std::cout << "\nBuild complete!\n";
...
vrt::v2::Dag::Hit vrt::v2::Dag::intersect(const Ray& ray, u8 depth, u32 root_index) const noexcept
{
    constexpr float INF = std::numeric_limits<float>::infinity();

    // Lustrzane odbicie promienia (Mirror ray)
    glm::vec3 O = ray.origin;
    glm::vec3 invD = ray.direction_inverse;
    std::uint8_t a = 0; // maska inwersji osi
    ...
    u32 current_node_idx = root_index; // Zaczynamy od głównego indeksu korzenia
    ...
    while (true)
    {
        // 1. ODCZYT Z 32-BITOWEGO DESKRYPTORA (Tylko jeden surowy odczyt z pamięci!)
        u32 desc = nodes_[current_node_idx];
        std::uint8_t true_oct = oct_idx ^ a;
        u32 child_chunk = (desc >> (4 * true_oct)) & 0xF;

        // 2. Jeśli dany oktant istnieje (Valid Bit zapalony)
        if (child_chunk & 0b1000)
        {
            // Odkodowujemy pozycję wskaźnika
            u32 offset = child_chunk & 0b0111;

            // Fizyczny wskaźnik do Dziecka leży zaraz za deskryptorem
            u32 child_ptr = nodes_[current_node_idx + 1 + offset];

            // Jeśli to nie jest sam dół drzewa, schodzimy niżej
            if (depth > 1)
            {
                float t_next = tx;
                ...
                // Odkładamy OBECNY INDEKS na stos
                ...
                // Skok w pamięci do nowego węzła
        ...
        while (true)
        {
            ...
            // POP (Wyjście z obecnego sześcianu w górę)
            if (t_next > t_exit)
            {
                if (sp == 0)
                    return { .t = INF, .normal = glm::vec3{0}, .voxel = {.rgbe = 0} };
                ...
                current_node_idx = frame.node_idx; // Odtwarzamy indeks z ramki stosu
                tx = frame.tx;
                ty = frame.ty;
                tz = frame.tz;
                t_exit = frame.t_exit;
